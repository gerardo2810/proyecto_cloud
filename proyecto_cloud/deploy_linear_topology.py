...
def desplegar_topologia_lineal(nombre_topo, vms, imagenes, usuario, rol):
    ...
    for i in range(num_vms - 1):
        ...
        logger.info(f"🌐 VLAN creada: vlan{vlan_id}_{nombre_topo}")
    ...
    for idx, vm in enumerate(vms_info):
        ...
        logger.info(f"🔧 Interfaces de {vm['nombre']}: {vm['interfaces']}")  # <-- Aquí
        logger.info(f"🚀 Lanzando VM {vm['nombre']} en {vm['worker']} con args: {arg_string}")
    ...
    print("\n🎯 PUCP DEPLOYER | Puertos VNC asignados - Topología Lineal:")
    for vm in vms_info:
