...
runcmd:
  ...
  - apt purge -y snapd lxd-agent-loader landscape-common apport ubuntu-advantage-tools popularity-contest friendly-recovery update-manager-core update-notifier-common manpages man-db info byobu ntfs-3g open-vm-tools usbutils os-prober || true
...
"""
    ...
    with open(f"{seed_dir}/meta-data", "w") as f:
        f.write(meta_data)
    with open(f"{seed_dir}/network-config", "w") as f:
        f.write(network_config)

    # Validación opcional (solo de user-data)
    try:
        run(f"cloud-init schema --config-file={seed_dir}/user-data")
    except subprocess.CalledProcessError as e:
        print(f"⚠️ Advertencia: YAML inválido según cloud-init schema. Detalles:\n{e}")

    print("NOMBRE VMMMMMMMMMMMMM------<" + nombre_vm)
    seed_img = f"/tmp/seed_{nombre_vm}.iso"
    run(f"genisoimage -output {seed_img} -volid cidata -joliet -rock {seed_dir}/user-data {seed_dir}/meta-data {seed_dir}/network-config")
    return seed_img
...
def crear_vm(nombre_vm, ovs_name, cpu, ram, almacenamiento, imagen, interfaces):
    ...
    if is_ubuntu:
        ...
        # Obtener tamaño de imagen base
        base_info = subprocess.check_output(f"qemu-img info --output=json {base_img_path}", shell=True).decode()
        import json
        base_size = json.loads(base_info)["virtual-size"] // (1024 * 1024)  # en MB

        if almacenamiento > base_size:
            run(f"qemu-img resize {disco_path} {almacenamiento}M")
        else:
            print(f"⚠️ Tamaño solicitado ({almacenamiento}MB) menor o igual al de la base ({base_size}MB). No se aplica resize.")
